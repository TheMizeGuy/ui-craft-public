...
| Utility dashboard (Data-dense, Executive summary, Real-time monitoring) | Density, tabular numerals, status color, chrome quiet in colour and confident in structure (edges, surface ladder, entity icons) | BI, ops/monitoring, finance internals, admin | Marketing surfaces, consumer onboarding | Density is a discipline, not a look: spacing rhythm and type caps (`references/design/02-typography.md`) or it collapses into noise |
| Dark-native (OLED dark, Terminal/CLI, Cinema dark) | True-dark surfaces, luminous status color, tabular numerals in data | Dev tools, media/streaming, trading, night-use apps | Print-first content, outdoor/high-brightness use, warmth-critical brands | Dark mode is designed, not inverted (`references/design/01-color-oklch.md` dark-surface craft); pure-black + neon everywhere is a tell, and so is its opposite -- a shelf-picked near-black with one 10%-alpha hairline for every boundary, no elevation and the accent stripped back to status colour is catalogue V13, the flat-terminal subtraction-only default (`references/aesthetic/06-substance-floor.md` § 1 is the floor it fails); Terminal/CLI: mono stays inside the actual terminal pane; mono + uppercase + tracking on chrome, labels or stats is catalogue T13 (maintainer-banned) |
...
| Dev tools / technical | Dark-native, docs-forward | Editor-dark + syntax accents, blue focus | Functional sans, tabular numerals; mono only inside code/log content | Light-only; slow perceived performance; ornament that carries no data -- panels, edges, icons and one saturated accent are structure, not decoration (`references/aesthetic/06-substance-floor.md`); mono leaking out of code panes into chrome |
...
| Internal tools / admin | Data-dense dashboard | Functional neutrals + status colors (reserved, `references/dataviz/02-color-jobs-and-validation.md`) | Clear, tabular numerals | Ornament that carries no data; panels, edges, item icons, a saturated brand accent and an elevation ladder are structure, not decoration (`references/aesthetic/06-substance-floor.md`); status colors leaking into series colors |
