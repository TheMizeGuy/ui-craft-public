...
The counts above are the *historical* source-catalogue figures, frozen at the 2026-07-07 merge. **Current totals are carried by the section headings themselves and are authoritative there:** 99 numbered base tells across sections 1-8 (C18, T15, L14, S12, W12, M7, I8, U13), 13 user-flagged visual tells (V1-V13), 35 platform tells (P15 / A10 / AM10), 20 Deep Cuts (D1-D20), plus the Strongest-10 ranks and the section-11 fingerprints. When you add a tell, update its section heading count and this line in the same edit.
...
**Since 2026 the median has a second face.** Where a model has learned to avoid the list above, its output converges on the opposite default: a flat, chrome-less, grey or near-black surface with one hairline for every boundary, no elevation, the accent stripped to status colour, badges and edges removed, the system stack at default weights everywhere. It is the near-black-plus-one-acid-accent look and the broadsheet-hairline look of the 2026 calibration lists, and clearing the tells below by deletion lands on it every time. This catalogue files it as V13; its counterweight, the floor a surface must clear whatever was removed, is `references/aesthetic/06-substance-floor.md`.

**The maintainer's standing rejections** (recorded preferences for this fleet's work, not corpus-confirmed tells): "rounded cards, Newsreader+JetBrains Mono pairing, colored-left-border sections, emoji indicators, large decorative serif numerals." Where the brief, the visual reference, or the target platform's own conventions name one of these, the brief wins (`references/aesthetic/01-point-of-view.md` section 2: the brief's own words always win, including when it asks for one of these looks) and the line carries `anti-slop-allow: <brief quote>`. Where the brief is silent, do not reach for them. On rounded cards specifically the catalogue's own finding is the repeated identical framed card (Strongest-10 row 1), not the radius.
...
- Cream/beige page background: hexes like `#faf8f5`, `#f5f1e8`, `#f3eee3` (OKLCH chroma 0.008 or more at hue 60-100 with L above 0.94; `#fdfbf7` and the C2 remedy `oklch(0.985 0.005 90)` sit below that chroma and read as off-white, not cream), or Tailwind `bg-stone-50` / `bg-amber-50` / `bg-orange-50` used as the page surface.
- Serif display face for headings: Instrument Serif, Fraunces, Playfair Display, Cormorant, Spectral, DM Serif.
- Sage / forest-green as the brand color (`#15573a`, emerald/green 700-900). Per the 2026-07 drift check the accent detail is shifting toward rusty-orange in fresh output. The terracotta or warm-clay variant most often lands near `#D97757`, which is Anthropic's own Claude-interaction accent, so on a user's brief it reads as a tell twice over; flag it by hue (OKLCH hue 25-50 at chroma 0.12 or more on a cream ground) as well as by hex.
...
**Presence-flaggable tells -- a single instance is a finding.** Two classes fire on one occurrence, so do not wait for a cluster. The first is the specific, high-signal compositions that read as AI on sight:

- every verbatim-signature fingerprint in section 11 -- the badge pill with its "Introducing" copy, gradient text, the blur-blob and dot-grid class runs, the frosted nav, the `$45,231.89` stats row, the dashboard trinity, the social-proof toast, the non-functional chat widget, the unmodified SaaS landing sequence -- where the signature itself is the copy-paste. The convention compositions in that section (pricing table, two-button CTA, four-column footer, 404 page, testimonial cards, skeleton defaults, alternating feature rows, wave dividers, uniform spacing) are conventions real teams ship on purpose: they fire on their generic content ("Most Popular" plus a "20%" toggle over placeholder tiers, "Get Started" + "Learn More", "Sorry, we couldn't find...", "All rights reserved" over the same four social icons, the "Sarah Johnson" pool), or when three or more of them co-occur on one page -- section 11's own "using ALL of them together" test. The structure alone is a convention, not a tell;
- every member of the Strongest-10 (section 17), except that rows 1, 4 and 6 are uniformity fingerprints and fire only on their repeated signature: row 1 when the border+shadow+radius trio sits on every card of a surface that has two or more, or on three or more cards anywhere; row 4 when the tracked-uppercase overline sits above three or more headings or on every table header and section label; row 6 when three or more icons share the identical tinted container. One framed card, one deliberate uppercase label, one framed icon is a choice, not a tell;
- V5 gradient text on hero words; L6 the frosted-glass sticky nav; S3 the unmodified shadcn `<Card>` (the verbatim library class run `rounded-xl border bg-card text-card-foreground shadow-sm`, a default nobody touched, which is a different tell from Strongest-10 row 1: that row is the uniformity fingerprint of any framed-card trio repeated across a surface, and a single hand-authored framed card with its own tokens is neither); V13 the flat-terminal subtraction-only system; W11 a prose blob above the primary content; I8 no real imagery on a visual product;
- the cream + serif + sage combination (the top emerging tell).

The second is every hard defect or absence, which is a finding on one instance because it is a defect rather than a style: the platform accessibility rows (P4, P8, P10, P11, P12 and their Apple and Android equivalents A6, AM6, AM7), W1 lorem ipsum, S5 placeholder button copy, the literal S12 value, U3 `outline: none`, U7, U8, U13, L13, T1/T2/T3 as the declared primary face, section 12 missing states, section 14 token bypass, section 15 demo-ware.

**Concentration tells -- need >=3 identical treatments, or dominance of a surface.** The property-level style rows -- the remaining Color (C), Typography (T), Copy (W), and Micro (U) rows, including T15, W12 and L14 at the thresholds their rows state, D1-D20, L2/L4/L5/L7, M1/M5, S2 -- are only a tell when repeated. Weight by density: three or more identical treatments across a surface, or one treatment that dominates the surface. A single `rounded-lg` card, one `<em>`, one glassmorphism panel, or one gradient is almost never a finding on its own. A row named in neither list is a style row and needs concentration, except where its own row says otherwise: C9 fires only when applied everywhere without purpose, V3 and V11 are user-banned by default and fire on one instance, and M6 is an absence that fires whenever no `prefers-reduced-motion` handling exists.
...
**One finding per span.** A construct that matches a base tell, a section-9 row, a section-11 fingerprint and/or a Strongest-10 row is one finding, filed under the most specific code with the aliases listed on the finding's `Tell:` line. A frosted nav is one L6 finding that also names Strongest-10 #2 and the section-11 fingerprint, never three findings.
...
3. **A remediation may never leave the surface flatter than a human team would ship.** Removing a frame, a badge, an accent edge, an elevation step, a focal image or the last chromatic token clears the tell and fails the substance floor (`references/aesthetic/06-substance-floor.md` § 1). Every removal names the device that takes over its job, from that file's § 6 pairing table, or the finding files the removal as an open question for the owner.
...
| C3 | AI purple family (indigo / violet / purple / fuchsia) as primary, accent, or hero gradient stop, including Tailwind blue as primary (`blue-500` sits at hue 259.8) -- `#6366f1`, `#4f46e5`, `#8b5cf6`, `#a855f7`, `#9333ea`, `#d946ef`, i.e. OKLCH hue 255-325 at chroma >= 0.15 | Tailwind `indigo-500` is the demo color in every starter, and every "make it pop" reflex lands somewhere in the violet-to-fuchsia arc | Pick a hue outside the 255-325 arc and at least 30 degrees from the default you replace (the binding rule of the replacement table below): `oklch(0.58 0.16 40)` burnt-orange, `oklch(0.5 0.14 235)` deep-prussian, `oklch(0.55 0.13 145)` workshop moss. Document why this hue and not another |
...
| C5 | Purple-to-pink gradient hero `from-purple-500 to-pink-500` | Trained on Tailwind landing demos and v0 output | Banned in serious products. If the brand demands gradient: rare combos like `oklch(0.7 0.18 30)` -> `oklch(0.5 0.2 350)` warm-blood, or skip the gradient and use a single bold hue with depth: a committed brand field plus a photograph, a product shot or a subject artefact, and atmosphere under 0.05 (`references/design/10-hero-and-section-architectures.md` § 3). Never a flat grey field |
| C6 | Blue-to-teal gradient `from-blue-500 to-teal-400` | Linear's hero clones | Linear earned that gradient with the rest of the design. Imitating one component without the system reads as cargo-culting. Use one committed brand hue with depth (a textured solid, a rim, or a photograph behind it), never a flat grey field |
...
| C12 | Identical 0.1-opacity shadow on every component | `shadow-sm` on every Card | Shadow communicates elevation, so an identical shadow carries none. Tune a three-level scale (`references/design/07-depth-and-overlays.md` § 1: card, popover, modal) so resting, raised and floating surfaces differ measurably; a resting surface may carry no shadow but still needs a findable boundary (`references/aesthetic/06-substance-floor.md` S3, S4). In dark, the raised level is a top-edge rim, not a drop. The tell is uniformity, not the presence of shadow |
...
| `#ec4899` pink-500 | `oklch(0.68 0.16 45)` apricot | 50.7 | `oklch(0.52 0.13 240)` ink-blue | 114.3 | `oklch(0.5 0.12 30)` russet | 35.7 |
...
**Mono discipline (standing maintainer directive, 2026-07-17):** the Mono column above exists for genuine code, log, raw-payload and terminal content. A monospace face reached for on a human-readable surface -- headings, body, labels, kickers, stats, prices, timestamps, nav, buttons -- is tell T13, and a premium mono face is still T13 when nothing selected it. Where the brief, the visual reference or a defined brand names a mono face, or the product is itself a terminal, console or code surface, that is the brand voice and the rule does not fire; record it once as `anti-slop-allow: brand face` and keep the rest of the type system tuned. The sanctioned replacement for "mono numerals" is tabular figures on the system/brand sans (`font-variant-numeric: tabular-nums` on web, `.monospacedDigit()` in SwiftUI -- both render SF Pro/the sans, not a mono family). Modern system faces (SF Pro on Apple platforms, the system stack or brand sans on web) with weight + color hierarchy are the default voice for UI text.
...
| L14 | Text-only section run: three or more consecutive sections whose only children are headings and paragraphs, or a first viewport that is only text on a surface that is not an article | A section is cheap to fill with prose and expensive to compose; under length pressure every section degrades to text | Every second section carries a non-text device (a figure, a table, an image, a control, a chart, an entity list with icons) and the first viewport carries the focal visual (`references/aesthetic/06-substance-floor.md` S5, `references/design/12-copy-placement-and-volume.md` § 4). Never fixed by adding prose: the rule is a non-text child, not a word count |

**Detection cues (layout):** for the wide-display failure, read the page shell's width rule and the utilisation at 1920 and 2560: a shell capped at `max-w-7xl` or under about 1600px and centred on a control, content, reference or dashboard surface uses half of a 2560 display, and the fix is a fluid shell that spends the width (extra `auto-fit` columns, a supporting pane, a wider table; `references/review/05-density-and-economy.md`, `references/usability/05-app-shells-and-content-layout.md` § 3b), never a wider centred cap (D11, L13, and review/05's HIGH row at under 60%); for L14, count each `section`'s children matching `img, picture, svg, video, canvas, table, form, ul, ol, dl, button, figure, [role=list]` and flag a run of three or more sections with none, and a first viewport with none on a non-article surface; compare the page sequence against the SaaS template regardless of product type; measure the share of sections using `text-align: center` + `mx-auto` (>80% is centered-everything); check whether a bento/grid is content-justified or decorative; look for varied column widths (all equal = a tell); count `Card` containers vs alternative grouping devices; match the nav against `bg-white/80 backdrop-blur-md border-b sticky`; check for identical section padding on every section; look for auto-scrolling logo bars, counter animations, an L-R-L-R alternation pattern, and wave/curve SVGs between sections. For L13, grep the stylesheet for `width:` with a `px` value on a layout container (as opposed to `max-width`; a px width declared only inside a `@media (min-width: X)` block at or above that width is a cap, not a fixed shell), a `repeat(<n>, <fixed>)` grid template, `100vh` anywhere, `position: fixed` on a bottom bar with no `env(safe-area-inset-bottom)`, and the absence of a `<meta name="viewport">` tag (the absent tag is L13; P4 is the tag that disables zoom); then check whether any `@media` rule is restyling a *component* rather than the page shell, which is the P1 container-query failure.
...
The shadcn/ui community joke is that every site looks the same because nobody overrides the defaults. The list below is what you must change for shadcn to stop reading as shadcn. The init has two generations: the current (Tailwind v4) scaffold writes OKLCH neutrals and `--radius: 0.625rem`; pre-v4 wrote the HSL values and `0.5rem`. OKLCH notation is not evidence of a tuned theme; compare the values.

| shadcn default | What to change |
|----------------|----------------|
| `--background: oklch(1 0 0)` (current init; pre-v4 HSL `0 0% 100%`, i.e. `#fff`) | OKLCH off-white per surface (see C2) |
| `--foreground: oklch(0.145 0 0)` (current init; pre-v4 HSL `222.2 84% 4.9%`, near-black ink) | Tuned ink-tone matching the warm/cool brand temperature |
| `--primary: oklch(0.205 0 0)` (current init; pre-v4 HSL `222.2 47.4% 11.2%`, the navy) | Brand accent -- never the neutral or navy default |
| `--radius: 0.625rem` (10px, current init; `0.5rem` = 8px pre-v4) | Radius identity per POV (see S2) |
...
| W11 | Prose blob above the primary content: more than the lede budget of running prose (25 words on product and marketing surfaces, 40 on content and reference pages; `references/design/12-copy-placement-and-volume.md` § 3) above the first data, tool, product or entity element on first paint, or a paragraph added to a hero that already carries a headline, a subtext and an action. The four-paragraph search-copy introduction above a data table is the canonical case. The statement hero is not this tell: where the prose IS the page's primary content, one paragraph may take the headline's slot at display scale with nothing below it being pushed down, recorded in the copy map (`references/design/12-copy-placement-and-volume.md` § 2 and § 6) | An "explain the page" or search-copy instruction is satisfied by the shortest path, a block at the top of the layout, and every later review round adds a clause rather than moving one | The product first: H1, identity block, one lede, one action. Explanation, methodology and search copy move below the primary content into a headed section, behind a `details` whose summary names its content, or onto their own route. Presence-flaggable; the finding carries the word count and names the element taken as primary |
| W12 | Orphan paragraph: a `p` of running prose whose nearest sectioning ancestor carries no heading, whose parent is `main` or `body`, or whose siblings are components of another kind (a card grid, a table, a form, a chart) | The model writes explanation as it emits components and drops it at the cursor; nothing in the loop asks where the paragraph lives | Every paragraph gets a home: a headed section, a container of its own kind, a measure of 45 to 75ch (`references/design/12-copy-placement-and-volume.md` § 1). Prose that explains a component becomes its caption, helper or empty state. Fires at two or more orphans, or one over 60 words |

**Detection cues (copy):** for W11, identify the primary content element (the first `table`, `form`, entity-card list, chart root or non-nav control inside `main`) and sum the words of running prose before it against the `design/12` lede budget; for W12, walk each `p` up to its nearest `section`, `article` or `main` and flag those with no preceding heading inside it or with component siblings; search for lorem ipsum and "the future of X"; flag non-specific value propositions; measure "Powered by AI" / "AI-powered" density; scan for SaaS-speak verbs ("leverage", "supercharge", "seamless", "streamline"); grep for the known fake-testimonial names ("Sarah Johnson", "Michael Chen", "CEO at TechCorp"); flag "Welcome back!" and "Get started today!" microcopy. Additional generic microcopy AI reuses on every project: "Get started today!", "Unlock the power of...", "Join thousands of satisfied users", "We're here to help", "Stay in the loop", "Transform the way you...". And marketing-speak leaking into functional UI: "Supercharge your workflow" on a settings page, "Experience seamless integration" on connection settings -- write "Connect your GitHub account", not "Seamlessly integrate with GitHub."
...
| M3 | Page transitions over 400ms | "Cinematic" reflex | Snappy transitions: 200-400ms for page changes, 100-200ms for component state, 50-100ms for hover. Over 400ms feels broken |
...
| M6 | All animations ignore `prefers-reduced-motion` | Default Motion config | Gate every decorative animation behind `@media (prefers-reduced-motion: no-preference)` (motion is the opt-in) and keep a `reduce` override as the last line of defence, per `references/design/04-motion.md` section 7. Functional motion (loading, drag) can degrade to instant; decorative motion must disable cleanly |
...
**Detection cues (motion):** look for Framer Motion + IntersectionObserver on every section; a single global motion preset applied to all interactions; route-transition duration over 400ms; auto-scroll without pause-on-hover; identical hover treatment on all cards; no `prefers-reduced-motion` handling at all: decorative motion neither gated behind `(prefers-reduced-motion: no-preference)` nor overridden under `(prefers-reduced-motion: reduce)`; animation of `width` / `height` / `top` / `left` on hot paths. Timing budget: micro-interactions (hover, focus, toggle) under 200ms; medium transitions (panels, dropdowns) 200-300ms; full-screen up to 400ms; over 400ms feels broken (M3). See `references/design/04-motion.md`.

## 7. Image / Asset Tells (8)
...
| I8 | No real imagery on a visual product: every section is icon-cards, abstract shapes and type, with zero screenshots, photographs, product shots or commissioned illustration on a page whose subject is visual (a product, a place, a game, a person, an item catalogue) | An icon needs no asset pipeline and a real image needs one, so the default page has none. One of the most-cited specific complaints in the corpus ("most good websites are like 50% images if not more") | Show the real product: screenshots, photography, the domain's own icons on every entity row, commissioned illustration. Floor: one real image or figure per two sections on marketing and content pages, and an icon or thumbnail on every entity row where the domain has one (`references/aesthetic/06-substance-floor.md` S6). Presence-flaggable on a visual product; severity HIGH (see section 18) |

**Detection cues (image):** reverse-searchable stock images; Spline/Blender blobs with no content relationship; conversion-arrow elements; generic stock-photo section backgrounds; `bg-muted rounded-full` initials as the only avatar treatment; AI-artifact imagery (six-fingered hands, melted text); image `src` matching undraw / storyset / humaaans / blush, or the flat "corporate Memphis" illustration style (monochrome-accent floating limbs and blob figures). The structural tell from the corpus, **no real images at all**, is I8 above: count real images and figures per section and icons per entity row, and flag a visual product at zero.
...
| U13 | Icon-only control with no accessible name: a `<button>` or link whose only content is an `<svg>` or icon glyph, with no `aria-label`, no visually hidden text, and no `<title>` for the name to come from | The icon reads as self-explanatory in the screenshot the model is composing for, and nothing in a visual pass notices the missing name | Name every icon-only control: `aria-label="Close"` on the control, or visually hidden text inside it, with `aria-hidden="true"` on the decorative `<svg>`. WCAG 4.1.2 Name, Role, Value. Severity CRITICAL (see section 18) |

**Detection cues (micro):** search for `:` in label text; redundant placeholder label-restate; `outline: none` without a `:focus-visible` replacement; mouse and keyboard focus treated identically; `cursor: pointer` on static content; tooltips without a `:focus-visible` trigger or that use `title=`; two close affordances on one modal; a `<Spinner />` with no text; color-only state differentiation; locale-naive numeric dates; raw `Number.toString()` output without thousands separators; naive singular/plural string interpolation; an icon-only `<button>` or `<a>` whose only child is an `<svg>` or icon element, with no `aria-label`, visually hidden text, or `<title>` (U13).

## 9. Specific Visual Tells (user-flagged) (13)

These were surfaced by users flagging concrete on-screen shapes -- several restate the user's verbatim rejection list (colored-left-border sections, emoji indicators, decorative serif numerals). They overlap with the component fingerprints in section 11 but are catalogued here as discrete, screenshot-detectable patterns with a severity each. Every row names an ornament applied regardless of meaning; a device that encodes selection, severity, rarity or category on the rows that carry it (a selected-row edge, a rarity-coloured icon frame, a real status badge) is substance, not the tell (`references/aesthetic/06-substance-floor.md` § 4).

| # | Pattern | Description | Detection | Severity |
|---|---------|-------------|-----------|----------|
| V1 | Oval/pill icon backgrounds | Icons wrapped in `rounded-full bg-primary/10 p-3` or `bg-muted rounded-lg p-2.5`, creating identical tinted ovals/pills behind every icon in a feature grid | Search for `rounded-full` / `rounded-lg` wrapping icon components with tinted backgrounds | HIGH at three or more identical tinted wrappers (Strongest-10 #6); never on a framed icon that encodes identity or state |
...
| V12 | Emoji as interface chrome | Emoji used as status dots, list bullets, section icons, or sparkles and rockets in buttons and headings, outside prose -- the user-banned "emoji indicators" pattern, and data-confirmed as a tell (file 02: emoji *as UI*, not in copy) | Grep markup and JSX for emoji code points outside paragraph text, whether literal or written as character references (`&#x1F680;`) or JS escapes (`\u{1F680}`); emoji inside prose is not this tell | MEDIUM (data-confirmed, medium FP risk: emoji in copy is not this tell) |
| V13 | Flat-terminal subtraction-only system | The 2026 default: a grey or shelf-picked near-black ground (`#0B0B0B`, `#111`, chosen rather than derived from the brand hue), panels with no elevation and a single hairline at 10% alpha or less as the only boundary, badges, frames and accent edges removed, the accent under chroma 0.10 or absent from first paint, colour reserved for status, the system stack at default weights on every surface. The system can be described entirely as things it does not do (taste smell test 9.10). Rendered a data product flatter on six consecutive campaigns before it was named | Substance checks S3 and S4 failing together with S1 or S2 on the same surface (`references/aesthetic/06-substance-floor.md` § 1): measure the hairline's contrast against both surfaces, the accent's chroma, and count the surface levels; `scripts/measure_substance.js` prints all three | HIGH (owner directive 2026-09-16; presence-flaggable) |
...
| P3 | `100vh` on mobile causing content behind browser chrome (use `100dvh`) | MEDIUM; HIGH on a modal, sheet, drawer or full-height form (responsive/01 section 7) |
...
**Gradient text on hero headings** (= V5). `bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent` applied to one or two words ("Build **Better** Faster"). The gradient-text span is a v0/Claude signature. *Instead:* size contrast, weight contrast across the whole headline, or serif/sans mixing within it. Not a single accented word in another colour, weight or italic: that is T15, the same reflex without the gradient. The typography should be the design, not a CSS gradient trick. **HIGH.**
...
- Pure black `#000000` backgrounds cause halation on OLED: use the OKLCH warm/cool darks from C1 (`oklch(0.12 0.005 280)`, `oklch(0.14 0.008 60)`). **HIGH** (this is C1).
...
|---|---------|--------------------|
| D1 | Border-radius `8px` or `10px` on every interactive control | The shadcn `--radius` defaults (`0.5rem` pre-v4, `0.625rem` current). Pick `0`, `2`, `6`, or `12` deliberately |
...
|------|-------------|----------------------|---------------|---------|
| 1 | `rounded-xl shadow-sm border` on every Card | `rounded-xl shadow-sm border` on every `Card` | Count Card components with identical border+shadow+radius treatment; fires on every card of a surface with two or more cards, or three or more cards anywhere. One framed card is a choice, not this tell | Commit the trio instead of deleting it: a radius the system states, a border colour that measures against both surfaces, an elevation step that differs by panel role (`references/aesthetic/06-substance-floor.md` S3, S4). Framed panels with real edges are the reference for data products; the tell is one unexamined treatment on every card, and removing containment from data panels is the flatter default, not a fix (S3) |
...
| 4 | All-caps tracking-wide subtitles / overlines | `text-xs uppercase tracking-wider` on table headers, section labels | Search for the `uppercase tracking-wider` pattern; fires when it is template chrome, an overline above three or more headings or on every table header and section label. One deliberate uppercase label in the brand sans at tuned tracking is a choice | Small-caps via `font-feature-settings: 'smcp'` at `tracking-wide` (not `widest`), which keeps screen-reader pronunciation intact, where `text-transform: uppercase` can make some screen readers spell the word letter by letter, or drop the overline and let the heading carry the section (T5). HIGH only when this exact signature is present; otherwise T5's own MEDIUM applies |
...
| Severity | Tells |
|----------|-------|
| **CRITICAL (block ship)** | C15 hex-not-OKLCH, T1/T2/T3 Inter/Roboto/Newsreader+JBM as primary, T13 mono on human-readable text, S1/S2/S3 default shadcn unmodified, S5 "Button" copy, W1 lorem ipsum, S4 Lucide everywhere, U3 `outline: none` with no `:focus-visible` replacement, U4 no :focus-visible distinction, M6 ignored prefers-reduced-motion, U8 spinner without label, U13 icon-only control with no accessible name, L1 generic SaaS scaffold; platform: P4 disabled zoom, P8 div-onclick, A6 missing VoiceOver labels, AM6 missing Compose semantics, AM7 config-change crash |
| **HIGH (fix before launch)** | Cream+serif+sage combination, C1/C2 pure black/white, C3/C4 AI-purple-family primary + indigo/teal pairing, L13 fixed-pixel shell that cannot resize, C5/C6 default gradients, C18 neon/colored-glow shadow, T4 Geist unmodified, T8 untuned letter-spacing, T10 centered hero, L2 centered everything, L4 equal-column grid, L6 frosted nav verbatim, L10 generic dashboard, S6 placeholder-as-label, S11 "Most Popular" badge, W2/W5 SaaS-speak, U7 double-close-button, missing empty/error states, demo-ware non-functional controls, hardcoded-values-vs-tokens drift, V1 oval icon backgrounds, V5 gradient hero text, V7 social-proof toast, V8 non-functional chat widget, V11 pulsing live badge, V13 flat-terminal subtraction-only system, W11 prose blob above the primary content, I8 no real imagery on a visual product, C14/U9 colour as the only signal (WCAG 1.4.1), Strongest-10 rows 1-10 when the row's exact CSS signature is present (rows 1, 4 and 6 at the repeated-signature threshold § How to apply states), scoped the way the stats-row note at section 11 scopes S12/L10; otherwise the base tell's own class applies (T5 MEDIUM, skeleton defaults MEDIUM, C13 LOW); platform: P9/P10/P11/P12/P13/P15 web a11y, A2/A3/A4/A5/A7 Apple, AM3/AM8/AM9 Android |
| **MEDIUM (fix in the next pass)** | C7 untuned neutrals, C9 glassmorphism (only when everywhere/without purpose), C12 uniform shadow, C16 mirror dark mode, T5 all-caps overlines, T6 display numerals, T7 two-weight stack, T11 single-line giant headline, T12 14px body, T14 stepped type scale where a fluid ramp belongs, L3 bento-as-default, L7 padding overload, L11 alternating L-R features, M1 fade-on-scroll, M2 wrong-energy motion, S7 default toast, U3 untouched browser-default ring, U10 ambiguous dates, dark-mode bias, uniform spacing scale, V2/V3 edge/left-border highlights, V4 badge pill, V6 two-button CTA, V10 testimonial cards, V12 emoji as UI, T15 template chrome strings, L14 text-only section run, W12 orphan paragraphs, I7 flat-vector illustrations, 404 formula, skeleton defaults, form-loses-state, modal overuse, search-without-debounce; platform: P1/P2/P3/P5/P6/P14, A1/A8/A10, AM1/AM2/AM4/AM5 |
...
This table says what a tell costs once it has fired; whether it fired is § How to apply's question, and a row carries its class only after that section says the tell matched. In particular the section-11 convention compositions -- pricing table, two-button CTA, four-column footer, 404 page, testimonial cards, skeleton defaults, alternating feature rows, wave dividers, uniform spacing -- match on their generic content or when three or more co-occur on one page. Their structure alone is a convention real teams ship, not a match, and a class above is not a licence to file one on sight.
