...
| Color and visual hierarchy | Contrast, semantic color use, interactive emphasis, theme consistency, CTA priority, and whether the system HAS an accent and a surface ladder at all | Low-contrast text, too many accents, decorative-only saturation, unclear CTA priority, broken dark mode; and the absence signals, each of which needs the number beside it (`references/aesthetic/06-substance-floor.md` section 1): the brand accent absent from first paint or below the 0.10 OKLCH chroma floor with no stated reason; no perceivable boundary between adjacent surfaces (a tint step under 1.15:1 or an edge under 1.3:1, measured as a contrast ratio in pixels, never as an OKLCH delta-L); monochrome by accident (every element under chroma 0.03 with no containment doing the work instead); hierarchy carried in one channel where three of weight, size, colour, containment, space and iconography are available |
...
| Content quality | Clarity, microcopy, placeholder leakage, scannability, action language, and WHERE each paragraph sits relative to the thing the page is for | Lorem ipsum, generic marketing filler in product UI, labels that don't explain actions, truncation hiding meaning; and the placement signals (`references/design/12-copy-placement-and-volume.md`): running prose above the primary content over that surface's lede budget, or any paragraph inside the hero; orphan paragraphs, meaning a `p` of running prose whose nearest sectioning ancestor carries no heading, whose parent is `main` or `body`, or whose siblings are components of another kind; a run of three or more text-only sections; copy whose position on the page depends on a third-party slot rendering |
...
| HIGH | Serious quality regression or likely user friction | Overlapping controls, broken reflow, confusing hierarchy, animation impedes use, color-only status; **and waste at the same scale**: viewport utilisation under 60% with no second column, sidebar or reading-measure reason, or an action a person cannot associate with its object (thresholds and the page-length rule: `references/review/05-density-and-economy.md`); **and measured flatness at the same scale**: zero perceivable boundaries between surfaces on a data surface, an accent below the 0.10 chroma floor with no stated reason, or a text-only first viewport on a product or marketing page (`references/aesthetic/06-substance-floor.md` section 2); and W11, more running prose above the primary content than that surface's lede budget allows (`references/design/12-copy-placement-and-volume.md` section 3) |
...
**Flatness is not automatically TASTE.** Same ladder, same mechanism, opposite
direction: a page with no accent, no perceivable surface boundaries and no
imagery also renders perfectly, so a reviewer calibrating honestly against
examples about breakage files it as a stylistic preference and it dies in
triage. That is how six consecutive campaigns each rendered one data product
flatter than the last while every audit stayed green. Rank flatness by the same
rule as waste: carry the number (accent chroma, the boundary ratio in pixels,
surface levels, images per section) and it is a substance finding at the
severity `references/aesthetic/06-substance-floor.md` section 2 assigns.
Without a number it genuinely is TASTE, and that boundary is what keeps the
measured ones credible.
...
| `Recommended change:` | Always | A specific direction. Include the code when it is short enough to read inline. **A change that removes a device names the device that takes over its job**: a frame becomes a heading plus spacing or a measured edge, a badge becomes an inline status word, an accent edge becomes a selected-state fill. The working list is `references/aesthetic/06-substance-floor.md` section 6. A removal with no named successor is filed as an open question for the owner, never as a `Recommended change:` -- "remove X" on its own is how a remediation leaves a surface flatter than a human team would ship. This block is the canonical source of that rule; the architect, the team lead, `improve-ui` and `design-ui` restate it and cite this block |
...
- Design feels generic, without a measurement
- Visual language lacks personality, without a measurement
- More distinctive typography could help
- Palette feels safe or over-familiar, without a measurement

These can still be useful findings, but they must not crowd out real defects.

**The qualifier is load-bearing.** Attach the `references/aesthetic/06-substance-floor.md`
numbers -- accent chroma, surface levels and the ratio between them, the boundary
ratio, images per section, hue and weight counts -- and the same observation stops
being a taste note and becomes a substance finding at its measured severity.
"Palette feels safe" is TASTE; "the accent measures chroma 0.04 and appears in no
fill on first paint" is the HIGH that section 2 of that file assigns. Pre-classifying
these three lines as taste regardless of the evidence attached to them is the
mechanism that let a flat product pass review, so read the evidence before
routing the finding here.
