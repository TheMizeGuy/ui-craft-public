...
Cross-ref: `references/design/01-color-oklch.md` § Dark-mode surface craft for elevation on dark surfaces; `references/performance/03-css-perf.md` for the cost of `backdrop-filter`.
...
| Litmus test | **If the shadow is the first thing you notice, it's wrong.** Shadow is felt, not seen |
...
Dark mode: shadows barely register on dark backgrounds — elevation becomes a lightness delta instead (each layer up is a *lighter* surface). See `references/design/01-color-oklch.md` § Dark-mode surface craft.
