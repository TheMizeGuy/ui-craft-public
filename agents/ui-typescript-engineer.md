...
cd <root> && node node_modules/ts7/bin/tsc --noEmit 2>&1 | head -200
...
- **Every rework is clean under the project's TS7 gate** (`node node_modules/ts7/bin/tsc --noEmit`), and TS6-strict-compatible wherever the project still ships a TS6 emit lane. That is the compiler you just ran, so it is the standard you author against. Mentally verify. Don't ship code that requires `any` or `@ts-ignore`.
