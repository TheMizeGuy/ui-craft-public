...
- **No `model:` field in any agent frontmatter, and none on a dispatch.** The dispatching session chooses the model per dispatch (Opus 5 is the usual default for design, review and implementation); the invoking session orchestrates. Model selection is a session decision, never baked into the agent files or the skills, so it can move without touching either.
