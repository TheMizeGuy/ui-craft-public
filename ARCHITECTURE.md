...
├── agents/                    10 agents: 9 specialists + ui-team-lead, none carrying a model pin
...
├── references/                 60 self-contained knowledge files, 12 domains
...
│   ├── usability/{01-task-flows-and-journeys, 02-forms-and-error-recovery, 03-navigation-and-information-architecture, 04-states-feedback-and-affordances, 05-app-shells-and-content-layout}.md
│   ├── responsive/{01-fluid-and-intrinsic-sizing, 02-breakpoints-vs-container-queries, 03-zoom-orientation-and-adaptive}.md
│   ├── design/{01-color-oklch, 02-typography, 03-spacing-rhythm, 04-motion, 05-tailwind-v4, 06-shadcn-customization, 07-depth-and-overlays, 08-ux-writing, 09-token-drift-and-retints, 10-hero-and-section-architectures, 11-image-to-code-replication, 12-copy-placement-and-volume}.md
│   ├── aesthetic/{01-point-of-view, 02-distinctive-systems, 03-taste-checklist, 04-style-taxonomy, 05-brand-direction-and-reference-generation, 06-substance-floor}.md
...
|---|---|---|
| `design-ui` | `ui-craft-architect`, then on its output `ui-visual-reviewer`, `ui-anti-slop-auditor`, `ui-responsive-reviewer` and `ui-accessibility-reviewer` (Step 5b), with the preview rendered and measured first when a browser tool exists (Step 5a) | Single, then a four-reviewer audit |
...
| `improve-ui` | `ui-team-lead` to up to 7 specialists (visual + usability, anti-slop, accessibility, motion, responsive, perf, typescript), then the verifier, always last and never in parallel | Team lead orchestrates (dispatched as `general-purpose`, see below) |
...
| Agent | Reads (must) |
|---|---|
| `ui-craft-architect` | catalogue/01, aesthetic/* (06 is the substance floor its contract is written against), design/* (12 is the copy map's source), usability/01-03 and 05, responsive/*, accessibility/*, architecture/01, architecture/03, platform overlay for the target, dataviz/01-03 when the design contains charts or stats |
| `ui-visual-reviewer` | catalogue/01-02, review/01, review/02, review/04, review/05, review/06, review/07, usability/* (this agent owns usability and task flow; 05 for shells and content layout), design/01-04, design/06-08, design/10, design/12, aesthetic/01-03, aesthetic/06 (lens 18, measured with `scripts/measure_substance.js`), accessibility/*, architecture/01, architecture/03, platform overlay for scope, dataviz/01+02+04 when charts are present |
| `ui-anti-slop-auditor` | catalogue/01 (primary), catalogue/02 (primary), review/01, aesthetic/03 (high), aesthetic/06 (high: the floor a remediation must still clear), aesthetic/01-02 (context), design/06 (context), design/12 (context: the placement tells) |
...
Two finding classes added in 0.6.0 ride on the existing enum deliberately: substance findings (the visual reviewer's lens 18, `references/aesthetic/06-substance-floor.md`) are `dimension: visual` with `Substance:` in the title and their numbers on the `Measurement:` line, and doctrine findings (a reviewed repo's own docs, lints or tests that enforce flatness or text volume, produced by `scripts/audit_doctrine.mjs` in `review-ui` Step 3b and `improve-ui` Step 2c) are `dimension: visual` with ids `visual-doctrine-<slug>` and titles beginning `Doctrine:`. Neither needed a `schemaVersion` bump, and minting a `substance` or `doctrine` enum value would have.
...
**Review ledger, schema v3 (`.claude/ui-craft/last-review.json`, written in the reviewed repo by `review-ui`, `improve-ui` and `optimize-ui`, on every run, without asking; `design-ui` writes none):**
...
```json
{
  ...
  "measurements": { /* optional, v3: keyed by width, the measure_substance.js and measure_density.js numbers from a browser run, verbatim */ }
}
```

v3 (0.6.0) adds the optional `measurements` block: `{"1920": {"utilisation", "accentChroma", "accentRoles", "surfaceLevels", "imagesPerSection", "wordsBeforePrimary", "orphanParagraphs", "textOnlySections"}}`, written by `review-ui` and `improve-ui` on browser runs and carried forward by `optimize-ui`. It exists for one delta bucket, **SUBSTANCE REGRESSED**: when a later run's number at the same width is worse (accent chroma, accent roles, surface levels or images per section lower; words before the primary content, orphan paragraphs or text-only sections higher; utilisation under 60%), the run files a `visual` finding titled `Substance: regressed since <prior timestamp>` at HIGH with both numbers. Six campaigns on one product each stripped the previous campaign's edges, badges and accent, and nothing compared a number across runs.

A ledger is the union of prior findings across runs; the delta compares this run against the entries whose dimension it reviewed. The next run against that same scope matches its fresh findings against this file on `id` + `file` to produce the delta report: absent from the ledger is **NEW**; present in the ledger, absent now, and re-verified as actually fixed is **RESOLVED**; present in both at the same severity is **STILL OPEN**; higher severity now is **REGRESSED**; lower severity now is **IMPROVED**; a worse measurement at the same width is **SUBSTANCE REGRESSED**. `dimensions` is what keeps a narrow run from erasing a wide one: a prior finding whose dimension is not in this run's `dimensions` is carried forward unchanged rather than counted RESOLVED. A missing or unreadable ledger is treated as an empty prior run (everything reports NEW), never an error.
...
**The deterministic layer (0.6.0).** `scripts/scan_tells.mjs` is the cheap first pass the semantic auditor runs after, not instead of: it finds the exact-signature tells a regex can see (the purple classes and hexes, gradient text, the frosted nav, the shadcn defaults, the hero triplet, the fixed shell, `100vh`, `transition-all`, `outline: none`, uppercase overlines at three or more, and heuristic candidates for the 0.6.0 tells marked `heuristic: true`), applies the catalogue's presence and concentration rules per file, and suppresses any hit carrying `anti-slop-allow`. `design-ui` Step 6 runs it on the emitted code and the preview in place of hand greps; `review-ui` and `improve-ui` run it over the scope before dispatching. The structural tells (sameness, a subtraction-only system, flow defects) remain the agents' job, which is why the scanner is a layer and not a gate on its own.
...
## How the team lead is dispatched (inherited from both prior team leads)

Dispatch via `subagent_type: "general-purpose"` with the agent body inlined and every `${CLAUDE_PLUGIN_ROOT}` occurrence replaced with the resolved absolute plugin root. The lead needs the Agent tool, and `Agent` access depends on runtime tool grants and nesting depth. The full note is at the top of `agents/ui-team-lead.md` (§ How this agent is dispatched).
...
- **No `model:` or effort field in any agent frontmatter or skill dispatch.** Model selection is a session decision made per dispatch, never baked into the agent files or the skills, so it can move without touching either. Dispatching to the specialists is the default; for a small scope the invoking session may run a specialist's process itself, and says so in the report header.
- **Two floors, not one.** Every rule the plugin shipped before 0.6.0 was a rule against excess, and a model clears every such rule fastest by removing the device, which is how six campaigns rendered one product flatter each time. `references/aesthetic/06-substance-floor.md` (what a surface must have, with measurements) and `references/design/12-copy-placement-and-volume.md` (where words may sit, with budgets on every surface type) are the counterweights, and they bind every path: the architect's substance contract and copy map, the taste checklist's section 1b, the visual reviewer's lens 18, the anti-slop auditor's "what this surface needs added", the verifier's rule that a removal with no named replacement is an open question, and the rubric's canonical `Recommended change:` field rule. The catalogue's own tells V13, W11, W12, L14, T15 and I8 make the failures reportable inside the existing dimensions.
