/**
 * Ported into ui-craft (2026-07-16) from the Claude Code bundled dataviz skill's
 * palette validator so the plugin is self-contained after that skill is retired.
 *
 * Validate a categorical chart palette against the COMPUTABLE data-viz checks:
 * four of the six documented checks, plus one addition (4b). Checks 1 and 6 are
 * structural and are never evaluated here; the report says so on every run.
 *
 * Design-system-agnostic: feed it ANY palette's hex values plus the mode and
 * surface, and it computes, never eyeballs, the five measurements that can be
 * derived from color alone:
 *
 ...
 * structural rules the skill enforces, not measurable from hexes alone. An
 * ALL CHECKS PASS line therefore means "the four computable checks pass", never
 * "the palette is approved" -- the report's closing line restates this.
 *
 * Usage (node, any version >= 18):
 ...
 *
 * Usage (browser, as a plain or module script):
 ...
 *
 * Usage (import): the file deliberately carries NO import/export syntax, so a
 * bare `.js` extension parses identically on Node 18, 20, 22 and later, in a
 * browser, and inside a repo with or without a package.json. The API is exposed
 * as CommonJS exports and, in a browser, as `globalThis.validatePalette`:
 *
 *   const { validate, validateOrdinal, contrast } = require("./validate_palette.js");
 *   import { validate } from "./validate_palette.js";   // named CJS interop
 *
 * Earlier revisions used `export` keywords in a `.js` file with no package.json,
 * which is a hard SyntaxError on every Node before module-syntax auto-detection
 * (22.12). The dual-mode tail below is what keeps the documented command from
 * crashing on an LTS runtime.
 *
...
// The boundary lives INSIDE the public functions, not only in the CLI and
// browser entrypoints. validate(), validateOrdinal() and contrast() are all
// documented as importable, so a guard that only runs in an entrypoint is a
// guard any caller bypasses. Unguarded, parseInt("zz", 16) yields NaN, every
// comparison against NaN is false, and three of the five checks report "pass"
// on garbage -- the run fails OPEN, the one outcome a validator must never
// produce. These helpers throw TypeError instead: loud, and impossible to
// mistake for a passing report.
function normalizeColor(value, what) {
  const v = typeof value === "string" ? stripWs(value) : value;
  if (!isHexColor(v)) {
    throw new TypeError(`validate_palette: invalid ${what} ${JSON.stringify(value)} -- expected "#rrggbb"`);
  }
  return v;
}
function normalizePalette(palette) {
  if (!Array.isArray(palette) || palette.length === 0) {
    throw new TypeError('validate_palette: palette must be a non-empty array of "#rrggbb" strings');
  }
  const clean = palette.map((v) => (typeof v === "string" ? stripWs(v) : v));
  const bad = clean.filter((v) => !isHexColor(v));
  if (bad.length) {
    throw new TypeError(`validate_palette: invalid palette entr${bad.length === 1 ? "y" : "ies"}: ${bad.map((v) => JSON.stringify(v)).join(", ")} -- expected "#rrggbb"`);
  }
  return clean;
}
function normalizeChoice(value, allowed, what, fallback) {
  const v = value == null ? fallback : value;
  if (!allowed.includes(v)) {
    throw new TypeError(`validate_palette: ${what} must be one of: ${allowed.join(", ")} (got ${JSON.stringify(value)})`);
  }
  return v;
}
...
const contrast = (a, b) => {
  const [ca, cb] = [normalizeColor(a, "color"), normalizeColor(b, "color")];
...
function validate(palette, { mode = "light", surface, pairs = "adjacent" } = {}) {
  mode = normalizeChoice(mode, ["light", "dark"], "mode", "light");
  pairs = normalizeChoice(pairs, ["adjacent", "all"], "pairs", "adjacent");
  palette = normalizePalette(palette);
  surface = normalizeColor(surface ?? DEFAULT_SURFACE[mode], "surface");
...
function validateOrdinal(palette, { mode = "light", surface, pairs } = {}) {
  ...
  if (pairs != null) {
    ...
    // a check the caller asked for and never got.
    throw new TypeError('validate_palette: "pairs" does not apply to an ordinal ramp (its checks are adjacent-only)');
  }
  mode = normalizeChoice(mode, ["light", "dark"], "mode", "light");
  palette = normalizePalette(palette);
  surface = normalizeColor(surface ?? DEFAULT_SURFACE[mode], "surface");
...
function printReport({ report, ok }, { mode, surface, ordinal, n }) {
  ...
  if (ordinal) {
    ...
    console.log("  not evaluated: check 1 (fixed hue order) and check 6 (values come from the"
      + " documented palette).\n  Both are structural and unmeasurable from hex values alone;"
      + " confirm them by hand against the design system.");
...
// ── module surface ─────────────────────────────────────────────────────────────
// No `export` keyword anywhere in this file: a `.js` file carrying ESM syntax
// with no package.json beside it is a SyntaxError on every Node before
// module-syntax auto-detection (22.12), which is exactly the LTS runtime the
// docs tell agents to run this on. The object-literal form below is what
// Node's cjs-module-lexer needs to expose these as NAMED imports from ESM.
const IS_NODE = typeof process !== "undefined" && !!(process.versions && process.versions.node);
const IS_CJS = typeof module !== "undefined" && typeof require !== "undefined";

if (IS_CJS) {
  module.exports = { validate, validateOrdinal, contrast, isHexColor, splitColors, stripWs, printReport };
} else if (typeof globalThis !== "undefined") {
  globalThis.validatePalette = { validate, validateOrdinal, contrast, isHexColor, splitColors, stripWs, printReport };
}

// Loaded as ESM under a `"type": "module"` package.json: `module` and `require`
// are undefined, so neither the exports above nor the CLI below can attach.
// Say so and exit non-zero rather than exiting 0 having validated nothing.
if (IS_NODE && !IS_CJS && typeof document === "undefined") {
  console.error(
    'validate_palette: loaded as an ES module (a package.json with "type": "module" is in scope),\n'
    + "  so its CommonJS exports and CLI cannot attach. Copy it as validate_palette.cjs, or run it\n"
    + "  from a directory whose nearest package.json does not set \"type\": \"module\"."
  );
  process.exit(2);
}

// ── Node CLI ───────────────────────────────────────────────────────────────────
if (IS_CJS && require.main === module) {
  const args = process.argv.slice(2);
  const VALUE_FLAGS = new Set(["--mode", "--surface", "--pairs"]);
  const CHOICES = { mode: ["light", "dark"], pairs: ["adjacent", "all"] };
  const USAGE = 'usage: node validate_palette.js "#hex,#hex,..." [--mode light|dark] [--surface #hex] [--pairs adjacent|all] [--ordinal]';
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--help" || a === "-h") { console.log(USAGE); process.exit(0); }
      ...
      const v = val ?? args[++i];
      if (v === undefined) { console.error(`${a} requires a value`); process.exit(2); }
      opts[a.slice(2)] = v;
    }
  ...
  // --pairs widens the CVD/normal-vision pairlist, which an ordinal ramp does
  // not have. Accepting both would run the default adjacent checks and print a
  // success report for a check the caller asked for and never got.
  if (opts.ordinal && opts.pairs != null) {
    console.error("--pairs does not apply with --ordinal: the ordinal checks are adjacent-only"); process.exit(2);
  }
  const palette = splitColors(positional);
  if (!palette.length) { console.error(USAGE); process.exit(2); }
...
// Browser auto-run (plain <script src> or <script type="module">, both work now
// that the file carries no ESM syntax). Fires whenever the page has a
// data-palette attribute on <body>; omit it to load the module without auto-running.
if (typeof document !== "undefined") {
  ...
  if (b?.dataset.palette) {
    ...
    // data-pairs with data-ordinal is the same contradiction the CLI rejects.
    const badEnum = !["light", "dark"].includes(mode) ? `data-mode ${JSON.stringify(mode)}`
      : !["adjacent", "all"].includes(pairs) ? `data-pairs ${JSON.stringify(pairs)}`
      : (ordinal && b.dataset.pairs != null) ? "data-pairs with data-ordinal (the ordinal checks are adjacent-only)" : null;
    const badHex = [...palette, surface].filter((c) => !isHexColor(c));
    if (!palette.length || badEnum || badHex.length) {
      // Top level, no `return` here; skip validating instead.
